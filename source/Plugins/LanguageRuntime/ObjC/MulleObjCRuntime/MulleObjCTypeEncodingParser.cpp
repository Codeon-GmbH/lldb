//===-- MulleObjCTypeEncodingParser.cpp -------------------------*- C++ -*-===//
//
...
#include "MulleObjCTypeEncodingParser.h"
...
MulleObjCTypeEncodingParser::MulleObjCTypeEncodingParser(
...
std::string
MulleObjCTypeEncodingParser::ReadStructName(lldb_utility::StringLexer &type) {
  StreamString buffer;
...
std::string
MulleObjCTypeEncodingParser::ReadQuotedString(lldb_utility::StringLexer &type) {
  StreamString buffer;
  while (type.HasAtLeast(1) && type.Peek() != '"')
    buffer.Printf("%c", type.Next());
...
uint32_t
MulleObjCTypeEncodingParser::ReadNumber(lldb_utility::StringLexer &type) {
  uint32_t total = 0;
...
MulleObjCTypeEncodingParser::StructElement::StructElement()
    : name(""), type(clang::QualType()), bitfield(0) {}

MulleObjCTypeEncodingParser::StructElement
MulleObjCTypeEncodingParser::ReadStructElement(clang::ASTContext &ast_ctx,
...
clang::QualType
MulleObjCTypeEncodingParser::BuildStruct(clang::ASTContext &ast_ctx,
                                         lldb_utility::StringLexer &type,
                                         bool for_expression) {
...
clang::QualType
MulleObjCTypeEncodingParser::BuildUnion(clang::ASTContext &ast_ctx,
                                        lldb_utility::StringLexer &type,
                                        bool for_expression) {
  return BuildAggregate(ast_ctx, type, for_expression, '(', ')',
                        clang::TTK_Union);
...
clang::QualType MulleObjCTypeEncodingParser::BuildAggregate(
...
clang::QualType
MulleObjCTypeEncodingParser::BuildArray(clang::ASTContext &ast_ctx,
                                        lldb_utility::StringLexer &type,
                                        bool for_expression) {
  if (!type.NextIf('['))
    return clang::QualType();
...
// for us anyway
clang::QualType MulleObjCTypeEncodingParser::BuildObjCObjectPointerType(
    clang::ASTContext &ast_ctx, lldb_utility::StringLexer &type,
    bool for_expression) {
  if (!type.NextIf('@'))
    return clang::QualType();
...
clang::QualType
MulleObjCTypeEncodingParser::BuildType(clang::ASTContext &ast_ctx,
...
CompilerType MulleObjCTypeEncodingParser::RealizeType(
