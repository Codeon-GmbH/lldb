...
class lldb_private::MulleObjCExternalASTSource
    : public ClangExternalASTSourceCommon {
public:
  MulleObjCExternalASTSource(MulleObjCDeclVendor &decl_vendor)
  ...
  bool FindExternalVisibleDeclsByName(const clang::DeclContext *decl_ctx,
    ...
    if (log) {
      log->Printf("MulleObjCExternalASTSource::FindExternalVisibleDeclsByName[%"
  ...
  void CompleteType(clang::TagDecl *tag_decl) override {
    ...
    if (log) {
      log->Printf("MulleObjCExternalASTSource::CompleteType[%u] on "
...
private:
  MulleObjCDeclVendor &m_decl_vendor;
};

MulleObjCDeclVendor::MulleObjCDeclVendor(ObjCLanguageRuntime &runtime)
  ...
  m_external_source = new MulleObjCExternalASTSource(*this);
...
MulleObjCDeclVendor::GetDeclForISA(ObjCLanguageRuntime::ObjCISA isa) {
...
bool MulleObjCDeclVendor::FinishDecl(clang::ObjCInterfaceDecl *interface_decl) {
  Log *log(GetLogIfAllCategoriesSet(
      LIBLLDB_LOG_EXPRESSIONS)); // FIXME - a more appropriate log channel?
  ...
  if (log) {
    ...
    log->Printf("[MulleObjCDeclVendor::FinishDecl] Finishing Objective-C "
                "interface for %s",
  ...
  if (log) {
    ...
    log->Printf(
        "[MulleObjCDeclVendor::FinishDecl] Finished Objective-C interface");
...
uint32_t
MulleObjCDeclVendor::FindDecls(const ConstString &name, bool append,
  ...
  if (log)
    log->Printf("MulleObjCDeclVendor::FindTypes [%u] ('%s', %s, %u, )",
